                    ...
                    <div class="card-text-light text-center bg-dark pb-2 reseñas-peliculas reseñas-peliculas-dk">
                        <div class="card-body text-white">
                            ...
                            <form method="post" action="dobled.php">
                                ...
                                $select_post->execute([$publicaciones["id"]]);
                                ...
                                ?>
                                            ...
                                            <div class="info-estrellas">
                                                <button name="add" class="btn btn-primary transUp" onclick="mostrarAlerta()">Calificar</button>
                                                <h5><?= $total_reivews; ?> Reviews</h5>
                                            </div>
                                <?php
                                            $update_pass = $conne->prepare("UPDATE reseñas_peliculas SET rate = ? WHERE id = ? ");
                                            $update_pass->execute([$average, $publicaciones["id"]]);
                                            $update_reviews = $conne->prepare("UPDATE reseñas_peliculas SET reviews = ? WHERE id = ? ");
                                            $update_reviews->execute([$total_reivews, $publicaciones["id"]]);
                                        }
                                    }
                                }
                                ?>
                                ...
                                <?php if (empty($row["username"])) { ?>
                                    ...
                                    <div class="info-estrellas">
                                        <button class="miBoton btn btn-primary transUp" onclick="mostrarAlerta()">Calificar</button>
                                        <h5><?= $total_reivews; ?> Reviews</h5>
                                    </div>
                                <?php } ?>
                            </form>
                                ...
                                if ($select_post->rowCount() > 0) {
                                    while ($fetch_post = $select_post->fetch(PDO::FETCH_ASSOC)) {
                                        ...
                                        if (!empty($row["username"])) { ?>
                                            ...
                                            <div class="info-estrellas">
                                                <button name="add_series" class="btn btn-primary transUp" onclick="mostrarAlerta()">Calificar</button>
                                                <h5><?= $total_reivews; ?> Reviews</h5>
                                            </div>
                            </form>
                <?php
                                            $update_pass = $conne->prepare("UPDATE reseñas_series SET rate = ? WHERE id = ? ");
                                            $update_pass->execute([$average, $publicaciones["id"]]);
                                            $update_reviews = $conne->prepare("UPDATE reseñas_series SET reviews = ? WHERE id = ? ");
                                            $update_reviews->execute([$total_reivews, $publicaciones["id"]]);
                                        }
                                    }
                                }
